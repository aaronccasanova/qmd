...
import { MetadataFilterError, type MetadataFilter, type MetadataMatch } from "../src/metadata-filter.js";
...
function observeStatements(db: Database, onPrepare: (sql: string) => void): Database {
  return new Proxy(db, {
    get(target, property) {
      if (property === "prepare") {
        return (sql: string) => {
...
describe("listMetadata key window", () => {
  ...
  test("the key window is materialized once per statement, at the sizes the status views use", () => {
    const plansByStatement = new Map<string, string[]>();
    const observed = observeStatements(store.db, sql => {
      if (sql.includes("selected_keys AS")) plansByStatement.set(sql, planOf(sql));
    });

    listMetadata(observed, { keyLimit: 5, valueLimit: 3 });
    listMetadata(observed, { collection: "notes", keyLimit: 5, valueLimit: 3 });
    listMetadataKeys(observed, undefined, 10);
    listMetadata(observed);

    expect(plansByStatement.size).toBeGreaterThanOrEqual(6);
    for (const [sql, plan] of plansByStatement) {
  ...
  test("reads the whole report from one snapshot while another connection writes", async () => {
    ...
    try {
      ...
      const observed = observeStatements(store.db, sql => {
        ...
        // have been read and before the medians and values are.
        if (rewritten || !sql.includes("MIN(mv.number_value)")) return;
        rewritten = true;
